...
if ("\" -eq $InputFolder.substring($len-1,1)) {
  ...
  Write-Host "Input-Folder-Name parameter had trailing backslash which was stripped" 
}
...
If ( -not (Test-Path -path $InputFolder -PathType Container)) {
  If (Test-Path -path $InputFolder) {
    Write-Error "Input-Folder-Name parameter specified: '$InputFolder' is not a directory. Aborting!"
  } Else {
    Write-Error "Input-Folder-Name parameter specified: '$InputFolder' does not exist. Aborting!"
  }
  Usage $myInvocation.InvocationName
  exit 1
}
