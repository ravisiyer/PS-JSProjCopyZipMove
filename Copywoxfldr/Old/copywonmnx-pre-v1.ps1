...
function TrailingBackslash {
  ...
  Write-Host "Source/input folder/directory or file with name: '$InputFolder' ends with backslash."
  Write-Host "Note that robocopy seems to trip up if directory name is in quotes and ends in backslash."
  Write-Host "To keep this script file simple, I am not handling conditions of input folder/directory name",
   "ending in backslash."
...
Write-Host "Input parameter (folder) specified is: ", $InputFolder 
