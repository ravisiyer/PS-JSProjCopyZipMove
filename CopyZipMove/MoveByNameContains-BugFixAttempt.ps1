# MoveByNameContains-BugFixAttempt.ps1
#
...
.EXAMPLE
    .\MoveByNameContains-BugFixAttempt.ps1 -Path "C:\Projects" -FFNamePatternsToMove @('Backend', 'Frontend') -TargetFolder "Sorted"
    Moves any files or folders containing 'Backend' or 'Frontend' from 'C:\Projects'
    to 'C:\Projects\Sorted', with confirmation prompts.

.EXAMPLE
    .\MoveByNameContains-BugFixAttempt.ps1 -FFNamePatternsToMove @('Logs', 'Archives') -TargetFolder "Cleanup"
...
if ($Usage -or ($null -eq $FFNamePatternsToMove -or $FFNamePatternsToMove.Count -eq 0) -or ([string]::IsNullOrWhiteSpace($TargetFolder))) {    
    if ($args[0] -eq '/?') {
        Show-Usage
    } else {
