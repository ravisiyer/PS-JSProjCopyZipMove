...
Write-Host "robocopy '$InputFolder' '$OutputFolder' /S /MAXAGE:$MaxAge /NDL /L /XD node_modules .next .gradle intermediates", `n
...
robocopy $InputFolder $OutputFolder /S /MAXAGE:$MaxAge /NDL /L /XD node_modules .next .gradle intermediates

Write-Host "Actual copy (list option not used) command to be executed :", `n
Write-Host "robocopy '$InputFolder' '$OutputFolder' /S /MAXAGE:$MaxAge /NDL /XD node_modules .next .gradle intermediates", `n
$Choices = [System.Management.Automation.Host.ChoiceDescription[]] @("&yes", "&no")
$Choice = $host.UI.PromptForChoice("", "Proceed?", $Choices, 1)
...
robocopy $InputFolder $OutputFolder /S /MAXAGE:$MaxAge /NDL /XD node_modules .next .gradle intermediates
