...
Write-Host "robocopy '$InputFolder' '$OutputFolder' /E /XO /NDL /L "
...
if ( $Option -eq $WoExclOption  ) {
	robocopy $InputFolder $OutputFolder /E /XO /NDL /L /XD node_modules .next build .gradle
} Else {
	robocopy $InputFolder $OutputFolder /E /XO /NDL /L
}

Write-Host "Actual incremental backup command to be executed", `n
Write-Host "robocopy '$InputFolder' '$OutputFolder' /E /XO /NDL "
if ( $Option -eq $WoExclOption  ) {
	Write-Host $ExclFolders
}
# Write-Host `n
...
if ( $Option -eq $WoExclOption  ) {
	robocopy $InputFolder $OutputFolder /E /XO /NDL /XD node_modules .next build .gradle
} Else {
	robocopy $InputFolder $OutputFolder /E /XO /NDL
}
